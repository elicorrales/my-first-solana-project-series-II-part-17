...
if (process.argv.length < 4) {
  ...
  console.log('2ND KEYPAIR IS TO BE CREATED.\n\n');
  return;
}
...
(async () => {
  ...
  let acctToBeCreatedTransInstruction = SystemProgram.createAccount({
    ...
    programId: old_program.publicKey,
    lamports: 1000000, //<---min required to get past the "insufficient ..rent"
    space: 1
  })
  let transaction = new Transaction();
  transaction.add(acctToBeCreatedTransInstruction);
  await connection.sendTransaction(transaction, [payerKeypair, acctKeypair]);
  console.log('Done.');
